...
class CrowdSim(gym.Env):
    ...
    def render(self, mode='human', output_file=None):
        def detect_visible(robot_fov,human_fov,state1, state2, robot1=False, custom_fov=None):
            if self.robot.kinematics == 'holonomic':
        ...
        if mode == 'human':
            ...
            ax.set_xticks([-11, -10, -8, -6, -4, -2, 0, 2, 4, 6, 8, 10, 11])
            ax.set_yticks([-10, -8, -6, -4, -2, 0, 2, 4, 6, 8, 10])
            #ax.set_title('SMART-Lab Social-Aware-Navigation', fontsize = 22, color = 'blue')
            
            #set human and human_number
            arrow_style = patches.ArrowStyle("->", head_length=4, head_width=2)

            human_position = []
            for human in self.humans:
                human_circle = plt.Circle(human.get_position(), human.radius, fill=False, color='green')
                ax.add_artist(human_circle)
                
                human_position = human.get_position()
                #human_numbers = plt.text(human_position[0]-x_offset,human_position[1]-y_offset,str('p'),color='blue',fontsize=12)
                #ax.add_artist(human_numbers)
                #human_numbers = [plt.text(humans[i].center[0] - x_offset, humans[i].center[1] - y_offset, str(i),
                                                        #color='green', fontsize=12) for i in range(len(self.humans))]
            ...
            ax.add_artist(goal)
            #time = plt.text(-9, 9, 'Time: {}'.format(0), fontsize=16)
            #ax.add_artist(time)
            
            #time.set_text('Time: {:.2f}'.format(frame_num * self.time_step))
            #print('The Global time is' self.global_time)
            print(' ')
            print(' ')
            print("The Global Time is (%s)s" %(self.global_time))
            #for k in range(len(self.states)):
                #global_time = k * self.time_step
                #time=plt.text('-9','9','Time: {}'.format(global_time),fontsize=16)
                #ax.add_artist(time)
                    #for human in humans:
                        #ax.add_artist(human)
           ...
           # human_positions = [[state[1][j].position for j in range(len(self.humans))] for state in self.states]
            ...
            ax.set_xticks([-11, -10, -8, -6, -4, -2, 0, 2, 4, 6, 8, 10, 11])
            ax.set_yticks([-10, -8, -6, -4, -2, 0, 2, 4, 6, 8, 10])
            #ax.set_title('CADRL', fontsize = 22, color = 'blue')
            #ax.set_title('SMART-Lab Social-Aware-Navigation', fontsize = 22, color = 'blue')
            ...
            agent_thetas = []
            robot_states = []
            human_states = [[] for i in range(len(self.humans))]
            # compute attention scores
            if self.attention_weights is not None:
                attention_scores = [
                    plt.text(-5.5, 5 - 0.5 * i, 'Human {}: {:.2f}'.format(i + 1, self.attention_weights[0][i]),
            ...
            if self.robot.kinematics == 'unicycle':
                ...
                for i in range(self.human_num + 1):
                    ...
                    for state in self.states:
                        if i == 0:
                            agent_state = state[0]
                            agent_theta = np.arctan2(agent_state.vy, agent_state.vx)
                            agent_thetas.append(agent_theta)
                            robot_states.append(agent_state)
                        else:
                            agent_state = state[1][i - 1]
                            human_states[i - 1].append(agent_state)
            ...
            robot_fov = np.pi / 2
            human_fov = np.pi / 2
            robotX, robotY = robot_positions[0]
            robot_theta = agent_thetas[0]
            artists = []
            if robot_fov < np.pi * 2:
                FOVAng = robot_fov / 2
            ...
            for i in range(len(self.humans)):
                ...
                if detect_visible(robot_fov, human_fov, robot_states[0], human_states[i][0], robot1=True):
                    humans[i].set_color(c='g')
                else:
                    humans[i].set_color(c='r')
            ...
            def update(frame_num):
                ...
                robot_theta = agent_thetas[frame_num]
                robotX, robotY = robot_positions[frame_num]
                FOVAng = robot_fov / 2
                startPointX = robotX
                startPointY = robotY
                endPointX = robotX + radius * np.cos(robot_theta)
                ...
                for i, human in enumerate(humans):
                    ...
                    if detect_visible(robot_fov, human_fov, robot_states[frame_num], human_states[i][frame_num],
                                      robot1=True):
                        humans[i].set_color(c='g')
                    else:
                        humans[i].set_color(c='r')
